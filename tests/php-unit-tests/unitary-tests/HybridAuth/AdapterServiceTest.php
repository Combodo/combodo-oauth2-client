...
use Combodo\iTop\Oauth2Client\HybridAuth\AdapterFabrikService;
...
class AdapterServiceTest extends ItopDataTestCase {
	private AdapterFabrikService $oAdapterFabrikService;
	...
	private function MockAdapterFabrikService($sProviderName, array $aConfig, AdapterInterface $oAuth2, ?string $sExpectedAuthorizationState=null) {
		$this->oAdapterFabrikService = $this->createMock(AdapterFabrikService::class);
		AdapterFabrikService::SetInstance($this->oAdapterFabrikService);
	...
	public function testRefreshToken_ByGet() {
		...
		$this->oStorageInterface = $this->createMock(StorageInterface::class);
		$this->MockAdapterFabrikService('google', $aConfig, $oAuth2, $sAuthorizationState);
